...
test_that("The value_from and value_to arguments work as they should", {
  skip_on_cran()
  expect_true(all(aq_measurements(city="Hyderabad", value_from=10, limit=10)$results$value>=10))
  expect_true(all(aq_measurements(city="Hyderabad", value_to=10, limit=10)$results$value<=10))
})
